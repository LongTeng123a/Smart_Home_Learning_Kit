...
void setup() {
  ...
  servo_10.write(180);//set servo connected digital 10 to 0°
  delay(300);
...
void auto_sensor() {
  ...
  Serial.println(gas);
  if (gas > 700) {//if variable gas>700
    flag = 1;//set variable flag to 1
    while (flag == 1)//if flag is 1, program will circulate
    {
      ...
      if (gas < 100)  //if variable gas is less than 100
      {
        ...
        digitalWrite(7, LOW);
        digitalWrite(6, LOW); //fan stop rotating
  ...
  if (light > 300)//if variable light is more than 300
  {
    ...
    if (infrar == 1)// if variable infra is 1
    {
  ...
  if (water > 800)// if variable water is larger than 800
  {
    flag2 = 1;//if variable flag 2 to 1
    while (flag2 == 1)// if flag2 is 1, program will circulate
    {
      Serial.println("rain");//output "rain" in new lines
      servo_10.write(0);// set the servo connected to digital 10 to 180°
  ...
  } else//Otherwise
  {
    ...
    {
      servo_10.write(180);//set servo connected to digital 10 to 0°
      delay(10);
    }
  }
  soil = analogRead(A2);//assign the analog value of A2 to variable soil
  Serial.println(soil);
  if (soil > 50)// if variable soil is greater than 50
  {
    flag3 = 1;//set flag3 to 1
    while (flag3 == 1)//If set flag3 to 1, program will circulate 
    {
...
void door() {
  ...
  if (btn1_num >= 5)//if variablebtn1_num ≥5
  {
    Serial.print("+");
    passwd = String(passwd) + String("+");//Set passwd 
    pass = String(pass) + String("+");//set pass
  ...
  infrar = digitalRead(2);//assign the value of digital 2 to infrar
  if(infrar == 0 && (val != '1' && val != 't'))//if variable infrar is 0 and val is not 'l' either 't'
  {
    servo_9.write(0);//set servo connected to digital 9 to 0°
    delay(50);
  }
